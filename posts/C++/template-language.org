...
#define canonicalize(name, tempArg, call, valType)      \
  tempArgs                                              \
...
canonicalize(apply, template<typename F, typename... A>, apply<F,A...>, typename apply<F,A...>::value_type);
 #+end_src

 #+begin_src
 error: macro "canonicalize" passed 7 arguments, but takes just 4
 #+end_src
...
Ahora, sólo tenemos que cambiar ~canonicalize~ introduciendo una llamada a ~EXPAND~ cada vez que usemos un argumento:
