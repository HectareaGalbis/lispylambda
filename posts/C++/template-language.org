...
Pero esto tiene solución. Sólo tenemos que hacer una meta-función que evalúe por nosotros. Esta meta-función será el evaluador de T++ 2000.0 y lo llamaremos ~eval~. 

La definición de ~eval~ es tan simple como esto:
...
Observa que ~eval~ no es un nuevo ~struct~. Es una alias para ~T::eval_impl<>~. Esto es así porque queremos que ~eval<T>~ sea ya el objeto al que se evalúa ~T~. Si ~eval~ fuese un ~struct~ entonce ~eval<T>~ sería un nuevo tipo de dato de C++ que hereda del objeto al que se evalúa ~T~. Es definitiva, no queremos un nuevo tipo, queremos que sea literalmente el objeto devuelto por ~T~.

Para poder acceder fácilmente a sus atributos generamos los famosos alias:

#+begin_src C++
template<typename T>
using eval = T::eval_impl<> {};

template<typename T>
extern constexpr auto eval_v = eval<T>::value;
#+end_src

Probemos ahora a evaluar la constante ~box<5>~ usando ~eval~:

#+begin_src C++ :exports both :eval never-export :results output :includes '(<iostream> </home/hectarea/lispylambda/posts/C++/prueba2.h>)
std::cout << eval_v<box<5>> << std::endl;
#+end_src
...
A la hora de evaluar la función, primero se evalúan los argumentos de la implementación ~F~ mediante ~eval<AS>...~ y a continuación se evalúa de nuevo ~F<eval<AS>...>~ para obtener el resultado.
...
¡Hecho! Ahora sí, ~add~ es una función de T++ 2000.0. Podemos usar ahora ~eval~ para evaluar la función:

#+begin_src C++ :exports both :eval never-export :results output :includes '(<iostream> </home/hectarea/lispylambda/posts/C++/prueba2.h>)
std::cout << eval_v<add<box<3>,box<6>,box<10>>> << std::endl;
#+end_src
...
#+begin_src C++ :exports both :eval never-export :results output :includes '(<iostream> </home/hectarea/lispylambda/posts/C++/prueba2.h>)
std::cout << eval_v<  add<  add<box<3>,box<6>>,  box<10>  >  > << std::endl;
#+end_src
...
Finalmente, fíjate que no hemos definido ningún alias para nuestra función. Los únicos alias que necesitaremos son los ya definidos para ~eval~.
